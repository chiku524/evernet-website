...
await client.setVersioning('Enabled') // opt-in S3-style versions (count against quota)
await client.setLifecycle([
  { id: 'docs', enabled: true, prefix: 'docs/', noncurrentDays: 30, expirationDays: 365 },
])
await client.s3Delete('docs/old.bin') // delete marker when versioning on; else trash (30d)
await client.s3RestoreVersion({ key: 'docs/report.bin', versionId: '…' })
const { versions } = await client.s3ListVersions({ prefix: 'docs/' })
await client.s3DeleteBatch({ prefix: 'tmp/', permanent: false })
...
const status = await client.getStatus() // readiness / capabilities
```
